...
public class ModArmorItem extends ArmorItem {
    private static final Map<Holder<ArmorMaterial>, List<MobEffectInstance>> MATERIAL_TO_EFFECT_MAP =
            ...
            // Added all vanilla and custom effects
            // Alexandrite armor
            .put(ModArmorMaterials.ALEXANDRITE_ARMOR_MATERIAL,
                 List.of(new MobEffectInstance(MobEffects.JUMP, 200, 1, false, false),
                         new MobEffectInstance(MobEffects.GLOWING, 200, 1, false, false))).build();

    public ModArmorItem(Holder<ArmorMaterial> material, Type type, Properties properties) {
        super(material, type, properties);
    }

    // DEFAULT METHOD - Check Player's inventory -> If used full armor added effect
    @Override public void onInventoryTick(ItemStack stack, Level level, Player player,
                                          int slotIndex, int selectedIndex) {
        if (!level.isClientSide() && hasFullSuitOfArmorOn(player)) { evaluateArmorEffects(player); }
    }

    // CUSTOM METHOD - Added each effect founded on list. Ex: Alexandrite armor
    ...
    // CUSTOM METHOD - Player used full armor (Helmet, Chestplate, Leggings, Boots) -> Added effect
    ...
    // CUSTOM METHOD - Check player used armor material type (Ex: Alexandrite armor)
    ...
    // CUSTOM METHOD - Player used full armor (Helmet, Chestplate, Leggings, Boots)
